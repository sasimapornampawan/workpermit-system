...
import { CONTRACTORS, CONTRACTOR_KPIS } from '../data';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Card style={{ overflowX: 'auto' }}>
        ...
        {CONTRACTORS.map((c) => (
          <TableRow key={c.code} columns={COLS} minWidth={850}>
            ...
            <div style={{ fontFamily: MONO, fontSize: 11.5, color: tone(c.insTone).fg }}>{c.insurance}</div>
            <div style={{ fontFamily: MONO, fontSize: 11.5, color: 'oklch(0.45 0.02 265)' }}>{c.cards}</div>
            <div><Pill t={c.statusTone}>{c.status}</Pill></div>
          </TableRow>
        ))}
      </Card>
    </div>
